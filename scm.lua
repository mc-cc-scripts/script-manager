...
function scm:downloadGit(sourceObject, repository, targetDirectory, updateObj)
    ...
    local infoFileContent = self:readInfoFile(infoFileUrl)
    ...
    if infoFileContent then
        ...
        file = fs.open(
            targetDirectory ..
            sourceObject.name .. self.config[sourceObject.type .. "Suffix"] .. "/" .. self.config["backUpFile"], "r")
        if isBackupFile then
